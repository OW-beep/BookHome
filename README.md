...
- 本の登録（手入力／バーコードスキャンのデモ、出版社・定価・購入価格も記録可）
...
## 既存プロジェクトに統計機能を追加する場合

すでに `schema.sql` を実行済みのSupabaseプロジェクトを使っている場合は、
追加で **`supabase/migrations_002_stats.sql`** の中身をSQL Editorで実行してください
（books テーブルへの列追加と、reading_logs テーブルの新規作成のみで、既存データは消えません）。
