...
- **購入導線（楽天ブックス・Kobo電子書籍・Amazon）**：本の詳細画面に3つのリンクを表示（ISBNがあればISBNで、なければタイトルで検索。ステマ規制に対応したPR表示つき）
...
- **タイトル検索での登録**：図書館の本などバーコードが手元にない場合、タイトルで検索して候補から選んで登録可能（国立国会図書館サーチを優先使用、国内の小規模出版社の本にも強くなりました。見つからなければGoogle Booksにフォールバック）
...
- **読みもの（`/articles`）**：ログイン不要で読めるコラム記事。トップページ下部に控えめなリンクあり。サイトマップにも自動反映されます
...
1. Amazonアソシエイトで発行されたトラッキングID（例：`bookhome0c-22`）をコピー
2. Vercelの Environment Variables に `NEXT_PUBLIC_AMAZON_ASSOCIATE_TAG` として追加し、Redeploy
